...
{   
    class RobotModel
    {
        public:
            ...
            const MatrixXd & getJacobian(const VectorXd &q, const int & frame_id=PANDA_NUM_LINKS) 
            {
				Jacobian(frame_id, q);
				return j_;
			}
            const MatrixXd & getJacobianv(const VectorXd &q, const int & frame_id=PANDA_NUM_LINKS) 
            {
				Jacobian(frame_id, q);
				return j_v_;
			}
            ...
            const Vector3d & getEEPosition(const VectorXd &q, const int & frame_id=PANDA_NUM_LINKS) 
            {
				Position(frame_id, q);
				return x_;
			}      
            ...
            const Affine3d & getEETransformation(const VectorXd &q, const int & frame_id=PANDA_NUM_LINKS) 
            {
				Transformation(frame_id, q);
				return trans_;
			}
            ...
            const double   & getManipulability(const VectorXd &q, const int & frame_id=PANDA_NUM_LINKS)
            {
                Manipulability(frame_id,q);
                return mani_;
            }
            const VectorXd & getDManipulability(const VectorXd &q, const int & frame_id=PANDA_NUM_LINKS)
            {
