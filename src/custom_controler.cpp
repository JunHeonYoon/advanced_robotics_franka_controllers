...
#include <advanced_robotics_franka_controllers/custom_controler.h>
#include <cmath>
#include <memory>
...
bool custom_controler::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle)
{
...
void custom_controler::starting(const ros::Time& time) {
  start_time_ = time;
...
void custom_controler::update(const ros::Time& time, const ros::Duration& period) {
  const franka::RobotState &robot_state = state_handle_->getRobotState();
...
PLUGINLIB_EXPORT_CLASS(advanced_robotics_franka_controllers::custom_controler,
